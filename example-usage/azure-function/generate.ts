...
const getCallback = (fileType: string): FileTypeCallback => {
    if (fileType.toUpperCase() === "TIFF") {
        return (img): sharp.Sharp => img.tiff();
    } else if (fileType.toUpperCase() === "JPEG" || fileType.toUpperCase() === "JPG") {
        return (img): sharp.Sharp => img.jpeg();
    }

    return (img): sharp.Sharp => img.png();
};
...
export const generateRandomImageBufferBruteForce = async (options?: GenerateOptions): Promise<Buffer> => {
    ...
    const fileTypeCallback = options.fileTypeCallback ?? getCallback(options.fileType);
